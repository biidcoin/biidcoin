...
#if defined(NDEBUG)
# error "biidcoin cannot be compiled without assertions."
#endif
...
CBigNum bnProofOfStakeLimitV3(~uint256(0) >> 10);
...
const string strMessageMagic = "biidcoin Signed Message:\n";
...
#ifdef ENABLE_WALLET
// biidcoin: attempt to generate suitable proof-of-stake
...
{
    string strBlockFn = strprintf("biid%04u.dat", nFile);
...
bool LoadExternalBlockFile(FILE* fileIn)
{
    ...
    {
        try {
            CAutoFile biiddat(fileIn, SER_DISK, CLIENT_VERSION);
            unsigned int nPos = 0;
            while (nPos != (unsigned int)-1 && biiddat.good())
            {
                boost::this_thread::interruption_point();
                unsigned char pchData[65536];
                do {
                    fseek(biiddat, nPos, SEEK_SET);
                    int nRead = fread(pchData, 1, sizeof(pchData), biiddat);
                    if (nRead <= 8)
                    {
                        nPos = (unsigned int)-1;
                        break;
...
{
    RenameThread("biidcoin-loadbiid");
