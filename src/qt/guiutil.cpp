...
{
    // biidcoin: check prefix
    if(uri.scheme() != QString("biidcoin"))
        return false;
...
{
    // Convert biidcoin:// to biidcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("biidcoin://"))
    {
        uri.replace(0, 12, "biidcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "biidcoin.lnk";
}
...
{
    return GetAutostartDir() / "biidcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("biidcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  biidcoin-qt [" + tr("command-line options") + "]                     " + "\n";
