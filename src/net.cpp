...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "biidcoin " + FormatFullVersion();
...
void static StartSync(const vector<CNode*> &vNodes) {
    ...
    BOOST_FOREACH(CNode* pnode, vNodes) {
        ...
        if (!pnode->fClient && !pnode->fOneShot &&
            ...
            (pnode->nVersion < NObiidS_VERSION_START || pnode->nVersion >= NObiidS_VERSION_END)) {
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. biidcoin is probably already running."), addrBind.ToString());
        else
